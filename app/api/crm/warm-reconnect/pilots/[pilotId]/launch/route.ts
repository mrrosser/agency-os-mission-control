...
import { requestWarmReconnectPilotLaunchForUid } from "@/lib/crm/warm-reconnect-repository";
...
const postLaunch = withApiHandler(
  async ({ request, params, correlationId, log }) => {
    const user = await requireFirebaseAuth(request, log);
    if ([...new URL(request.url).searchParams.keys()].length > 0) {
      throw new ApiError(400, "Pilot launch does not accept query parameters.");
    }
    ...
    const result = await requestWarmReconnectPilotLaunchForUid({
      uid: user.uid,
  ...
  { route: "crm.warm_reconnect.launch.post", persistServerErrors: false }
);

export async function POST(
  request: Parameters<typeof postLaunch>[0],
  context: Parameters<typeof postLaunch>[1]
) {
  return noStore(await postLaunch(request, context));
}
