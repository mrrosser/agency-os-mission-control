...
import {
  GrowthResearchMetricsPayloadSchema,
  GrowthResearchReviewResponseSchema,
  GrowthResearchWeeklyReviewSchemaSchema,
  type GrowthResearchPaths,
  type GrowthResearchReviewRequest,
  type GrowthResearchReviewResponse,
  type GrowthResearchSnapshot,
} from "@/lib/growth-research-contract";

const execFileAsync = promisify(execFile);

export const DEFAULT_GROWTH_RESEARCH_REPORT_ROOT =
  "C:\\CTO Projects\\CodexSkills\\docs\\reports";
export const DEFAULT_GROWTH_RESEARCH_SCRIPT_ROOT =
...
export function getGrowthResearchPaths(
  env: NodeJS.ProcessEnv = process.env
): GrowthResearchPaths {
  const reportRoot = readEnvPath(
    env,
    "GROWTH_RESEARCH_REPORT_ROOT",
    DEFAULT_GROWTH_RESEARCH_REPORT_ROOT
  );
  const scriptRoot = readEnvPath(
    env,
    "GROWTH_RESEARCH_SCRIPT_ROOT",
    DEFAULT_GROWTH_RESEARCH_SCRIPT_ROOT
  );

  return {
    reportRoot,
    scriptRoot,
    reviewLedgerPath: path.join(reportRoot, "growth-research-review-ledger.json"),
    weeklyReviewSchemaPath: path.join(
      reportRoot,
      "growth-research-weekly-review-schema-latest.json"
    ),
    metricsJsonPath: path.join(reportRoot, "growth-research-metrics-latest.json"),
    trainingDatasetPath: path.join(
      reportRoot,
      "growth-research-training-dataset-latest.json"
    ),
    reviewScriptPath: path.join(scriptRoot, "record_growth_research_review.ps1"),
  };
}
...
function buildSnapshotDetail(args: {
  ...
  paths: GrowthResearchPaths;
}): { status: GrowthResearchSnapshot["status"]; detail: string } {
  const issues: string[] = [];

  if (args.schemaStatus === "missing") {
    issues.push("Weekly review schema has not been generated yet.");
  } else if (args.schemaStatus === "invalid") {
    issues.push(
      `Weekly review schema is invalid: ${args.schemaError || "parse failed"}.`
    );
  }
...
function parseReviewScriptOutput(stdout: string): GrowthResearchReviewResponse {
  ...
  for (const candidate of candidates) {
    try {
      return GrowthResearchReviewResponseSchema.parse(
  ...
  throw new Error("Growth-research review recorder returned non-JSON output.");
}

function getReviewShell(env: NodeJS.ProcessEnv = process.env): string {
  const configured = env.GROWTH_RESEARCH_REVIEW_SHELL?.trim();
  if (configured) return configured;
  return process.platform === "win32" ? "powershell.exe" : "pwsh";
}

export async function getGrowthResearchSnapshot(
  env: NodeJS.ProcessEnv = process.env
): Promise<GrowthResearchSnapshot> {
  const paths = getGrowthResearchPaths(env);
  const [reviewSchemaResult, metricsResult, reviewScriptAvailable] =
    await Promise.all([
      readValidatedJson(
        paths.weeklyReviewSchemaPath,
        GrowthResearchWeeklyReviewSchemaSchema
      ),
      readValidatedJson(paths.metricsJsonPath, GrowthResearchMetricsPayloadSchema),
...
export async function recordGrowthResearchReview(
  input: GrowthResearchReviewRequest,
  options: {
    reviewer: string;
    correlationId?: string;
    env?: NodeJS.ProcessEnv;
  }
): Promise<GrowthResearchReviewResponse> {
  const env = options.env || process.env;
  const paths = getGrowthResearchPaths(env);
  if (!(await pathExists(paths.reviewScriptPath))) {
    throw new Error(
      `Growth-research review recorder script not found at ${paths.reviewScriptPath}.`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Growth-research review recorder failed: ${details || "unknown error"}.`
    );
  }
}
