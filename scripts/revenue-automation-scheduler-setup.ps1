...
$jobs = @(
    ...
    [PSCustomObject]@{ Name = "revenue-weekly-brain"; Cron = (Get-EnvOrDefault -Name "REVENUE_WEEKLY_BRAIN_CRON" -Default "10 6 * * 1"); Uri = $weeklyUri; Body = (@{ templateIds = @("rts-south-day1", "rng-south-day1"); dryRun = $false; forceRun = $false; timeZone = $timeZone; processDueResponses = $true; requireApprovalGates = $true; runCloserQueue = $true; runRevenueMemory = $true; runWeeklyKpi = $true; runServiceLab = $true } | ConvertTo-Json -Compress) },
