...
import { POST } from "@/app/api/social/drafts/rng-weekly/worker-task/route";
...
describe("social drafts rng-weekly worker-task route", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SOCIAL_DRAFT_UID = "uid-rng";
    process.env.SOCIAL_DRAFT_RNG_WEEKLY_TIMEZONE = "America/Chicago";

    createDraftMock.mockResolvedValue({
      draft: {
        draftId: "d-rng-weekly",
        uid: "uid-rng",
        businessKey: "rng",
        channels: ["instagram_post", "facebook_post"],
        caption: "RNG Weekly Spotlight (2026-W09)",
        media: [],
      ...
      approvalUrls: {
        approve: "https://leadflow-review.web.app/api/social/drafts/d-rng-weekly/decision",
        reject: "https://leadflow-review.web.app/api/social/drafts/d-rng-weekly/decision",
      },
      warning: null,
    });
  });
  ...
  it("rejects requests without worker token", async () => {
    const req = new Request("http://localhost/api/social/drafts/rng-weekly/worker-task", {
      method: "POST",
  ...
  it("creates a weekly rng draft with idempotent week key", async () => {
    const req = new Request("http://localhost/api/social/drafts/rng-weekly/worker-task", {
      method: "POST",
      headers: {
    ...
    expect(typeof data.weekKey).toBe("string");
